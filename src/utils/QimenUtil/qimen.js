...
export class Qimen {
    constructor(year, month, day, hour) {
        ...
        this.hour = hour;
        this.result = {
            干支: this.gangzhi()[0] + "年" + this.gangzhi()[1] + "月" + this.gangzhi()[2] + "日" + this.gangzhi()[3] + "時",
            旬首: this.xun(),
    ...
    find_jieqi() {
        ...
        let date = this.year.toString() + "/" + this.month.toString() + "/" + this.day.toString();
        let closest = s_date.sort(function (a, b) {
            return Math.abs(new Date(a).getTime() - new Date(date).getTime()) - Math.abs(new Date(b).getTime() - new Date(date).getTime());
        })[0];
        //这里是序号加1
        let test = {
            true: jieqi_list[s_date[s_date.findIndex(item1 => item1 == closest) + 1]],
    ...
    daykong_shikong() {
        return {
            日空: Config.multi_key_dict_get(Config.guxu, Config.multi_key_dict_get(Config.liujiaxun_dict, this.gangzhi()[2]))["孤"],
            時空: Config.multi_key_dict_get(Config.guxu, Config.multi_key_dict_get(Config.liujiaxun_dict, this.gangzhi()[3]))["孤"],
        };
    }
    ...
    //奇門排局
    qimen_ju_name() {
        let find_yingyang = Config.multi_key_dict_get(Config.yingyang_dun, this.find_jieqi());
        let findyuan = Config.multi_key_dict_get(Config.findyuan_dict, this.gangzhi()[2]);
        let jieqicode = Config.multi_key_dict_get(Config.jieqidun_code, this.find_jieqi());
        let find_kok = {
            上元: jieqicode[0],
            中元: jieqicode[1],
            下元: jieqicode[2],
    ...
    zhishi_pai() {
        let yinyang = this.qimen_ju_name()[0];
        let kook = this.qimen_ju_name()[2];
        let yanglist = Config.new_list(Config.cnumber, kook).join("") + Config.new_list(Config.cnumber, kook).join("") + Config.new_list(Config.cnumber, kook).join("");

        let yinlist = Config.new_list_r(Config.cnumber, kook).join("") + Config.new_list_r(Config.cnumber, kook).join("") + Config.new_list_r(Config.cnumber, kook).join("");

        let yangpai = Config.zip_dict(
            Config.liuxun,
            Config.new_list(Config.cnumber, kook)
                .slice(0, 6)
                .map(i => i + yanglist.slice(yanglist.split("").findIndex(item => item == i) + 1, Config.len(yanglist)).slice(0, 11))
        );
        ...
        let yinpai = Config.zip_dict(
            ...
            Config.new_list_r(Config.cnumber, kook)
                .slice(0, 6)
                .map(i => i + yinlist.slice(yinlist.split("").findIndex(item => item == i) + 1, Config.len(yinlist)).slice(0, 11))
        );
    ...
    zhifu_n_zhishi() {
        ...
        let chour = Config.multi_key_dict_get(Config.liujiaxun_dict, this.gangzhi()[3]);
        ...
        let zhifu_gong = Config.zip_dict(
            Object.keys(this.zhifu_pai()),
            Object.values(this.zhifu_pai()).map(item => Config.gongs_code[item[hgan]])
        )[chour];

        let zhishi_gong = Config.zip_dict(
            Object.keys(this.zhishi_pai()),
            Object.values(this.zhishi_pai()).map(item => Config.gongs_code[item[hgan]])
        )[chour];
    ...
    tianyi() {
        ...
        try {
            star_location = Config.stars_gong_code[this.zhifu_n_zhishi()["值符星宮"][1]];
        } catch (IndexError) {
            star_location = "禽";
        }
    ...
    pan_sky() {
        ...
        return [Config.zip_dict(gong_reorder, gan_reorder), {[this.pan_star()[1]["禽"]]: this.pan_earth()[0]["中"]}];
    }
    ...
    pan_earth() {
        ...
        let new_gua = Config.new_list(Config.cnumber_order, kok).map(i => cnum_to_gua[i]);
        let earth = Config.zip_dict(new_gua, Config.yingyang_order[kok_yingyang]);
        let reverse_earth = Config.zip_dict(Config.yingyang_order[kok_yingyang], new_gua);
        let cnumngua = Config.zip_dict(Config.new_list(Config.cnumber_order, kok), Config.yingyang_order[kok_yingyang]);
    ...
    //九星
    pan_star() {
        let starting_star = this.zhifu_n_zhishi()["值符星宮"][0].replace("芮", "禽");
        ...
        return [Config.zip_dict(gong_reorder, star_reorder), Config.zip_dict(star_reorder, gong_reorder)];
    }
    ...
    //丁馬
    dinhorse() {
        let dinhorsedict = Config.zip_dict(["甲子", "甲戌", "甲申", "甲午", "甲辰", "甲寅"], ["卯", "丑", "亥", "酉", "未", "巳"]);
        let xun = Config.multi_key_dict_get(Config.liujiaxun_dict, this.gangzhi()[2]);
        return Config.multi_key_dict_get(dinhorsedict, xun);
    }

    //天馬
    moonhorse() {
        let moonhorsedict = Config.zip_dict(["寅申", "卯酉", "辰戌", "巳亥", "午子", "丑未"], ["午", "申", "戌", "子", "寅", "辰"]);
        return Config.multi_key_dict_get(moonhorsedict, this.gangzhi()[2][1]);
    }

    //驛馬星
    hourhorse() {
        let hourhorsedict = Config.zip_dict(["申子辰", "寅午戌", "亥卯未", "巳酉丑"], ["寅", "申", "巳", "亥"]);
    ...
    gong_sheng_wang_mu(pan) {
        ...
        //使得宫、干、生旺墓对应
        let find_gong_gan_twelve_luck_new = Config.zip_dict(Object.keys(pan), find_gong_gan_twelve_luck);
        //依照宫内地支筛选生旺墓
            ...
            for (let jj in find_gong_gan_twelve_luck_new[ii]) {
                ...
                for (let kk in find_gong_gan_twelve_luck_new[ii][jj]) {
                    if (ii != "中" && Config.gong_dizhi[ii].includes(kk)) {
                        gong_gan_twelve_luck[ii][jj][kk] = find_gong_gan_twelve_luck_new[ii][jj][kk];
                    }
                }
            }
        }
    ...
    //神
    getgtw() {
        let newgtw_order = ["地籥", "天關", "唐符", "風雲", "唐符", "風雲", "雷公", "風伯", "天曹", "五符"];
